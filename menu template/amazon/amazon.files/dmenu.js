...
function _dmsa(d_ddm, d_sii, is) {
    ...
    var st = {
        ...
        d_bi: bI ? dengine._dmkl(bI, pPrefix) : dengine._dmkl(menuBackImage, pPrefix),
        d_qy: parseInt(_dmsi("smColumns", d_sii, 0, smColumns, is)),
        d_qt: dengine.d_du(_dmsi("menuBorderWidth", d_sii, 0, menuBorderWidth, is)),
...
function dm_itemList2Tree(itemList) {
    ...
    for (var d_iy = 0; d_iy < itemList.length; d_iy++) {
        ...
        var d_iv = {
            text: itemList[d_iy][0],
            link: itemList[d_iy][1],
            target: itemList[d_iy][5],
            tip: itemList[d_iy][4],
            d_dii: [itemList[d_iy][2], itemList[d_iy][3]],
...
function _dmsf(d_co, dur, vis, d_ce) {
    if (d_ce.d_dfe < 40) {
        if (d_co.filters && d_co.filters[0]) {
            if (!d_ce.filter) {
                d_ce.filter = d_co.filters[0];
            }
            with(d_ce.filter) {
                enabled = 1;
                if (status != 0) {
        ...
        if (d_ce.filter && d_ce.filter.status) {
            d_ce.filter.stop();
        }
        if (vis) {
            var effects = [];
            var d_sz = dengine._dmos(d_co);
            switch (d_ce.d_dfe) {
                case 101:
                    effects[effects.length] = new dengine.lI7(d_co, d_dm[d_ce.d_mi].m[d_ce.d_qri.d_ci].d_dhz ? d_ce.vAlignReal < 0 ? 0 : 2 : d_ce.hAlignReal < 0 ? 3 : 1);
                case 100:
                    effects[effects.length] = new dengine.lI8(d_co);
                    break;
                case 102:
                    effects[effects.length] = new dengine.lI7(d_co, d_dm[d_ce.d_mi].m[d_ce.d_qri.d_ci].d_dhz ? d_ce.vAlignReal < 0 ? 0 : 2 : d_ce.hAlignReal < 0 ? 3 : 1, 1);
                    break;
                default:
                    ;
            }
            if (effects.length) {
                d_ce.filter = new dengine._dmga(effects);
                d_ce.filter.duration = dur / 1000;
                d_ce.filter.apply();
            }
        }
    }
    d_co.style.visibility = vis ? "visible" : "hidden";
    return d_ce.filter;
}
...
function _dmmh(id) {
    ...
    if (!d_ce.d_ddv && d_dmr.d_psi != d_ce.id) {
        with(d_co.style) {
            var fl = _dmsf(d_co, d_ce.d_qi, 0, d_ce);
            if (fl) {
                try {
                    fl.play();
                    d_ce.hideTimer = setTimeout("dm_startRemove(\"" + id + "\")", d_ce.d_qi);
                } catch (e) {
                    fl.enabled = 0;
                    dm_startRemove(id);
                }
...
function dm_startRemove(id) {
    ...
    var d_co = _dmoi(id);
    if (dmIframeEn) {
        _dmoi(id + "frame").style.display = "none";
    }
    d_co.style.display = "none";
    if (dmObjectsCheck) {
        _dmzz(d_ce);
...
function _dmsz(d_ce) {
    ...
    d_co.style.left = "0";
    d_co.style.top = "0";
    var smOffset = dengine._dmos(d_co);
    var d_cc = dengine._dmos(_dmoi(d_co.id + "tbl"));
    var d_uv = d_ce.d_qri;
    ...
    var rsmSize = dengine._dmos(_dmoi(d_rso.id + "tbl"));
    var d_uo = d_uv.ll4().parentNode;
    var d_us = dengine._dmos(d_uo);
    if (d_ddm.d_dcf && d_t == 1 && d_ce.d_le == 1) {
        d_us = _dmOIa(d_ce, d_uo);
    }
    if (d_ce.d_ct.hParent) {
        d_us[0] = rsmSize[0];
        d_us[2] = rsmSize[2];
    }
    if (d_ce.d_ct.vParent) {
        d_us[1] = rsmSize[1];
        d_us[3] = rsmSize[3];
    }
    d_us[0] -= d_ce.d_qoz;
    ...
    var x = 0,
        ...
        resize = 0;
    with(d_ce.d_ct) {
        w = (smW ? parseInt(smW) : d_cc[2]) + d_ce.shadowLen;
        h = (d_qhi ? parseInt(d_qhi) : d_cc[3]) + d_ce.shadowLen;
    }
    ...
    var tmpXY = _dmO1(!d_ce.d_dhz ? d_dsd[1] : d_dsd[0], !d_ce.d_dhz ? d_dsd[3] : d_dsd[2], !d_ce.d_dhz ? d_us[1] : d_us[0], !d_ce.d_dhz ? d_us[3] : d_us[2], !d_ce.d_dhz ? h : w, !d_ce.d_dhz ? d_ce.d_ct.vAlign : d_ce.d_ct.hAlign, !d_ce.d_dhz ? !d_rsv.d_dhz : d_rsv.d_dhz);
    if (d_ce.d_dhz) {
        x = tmpXY.xy;
        resize = d_cc[2] + d_ce.shadowLen != tmpXY.d_sz;
        w = tmpXY.d_sz;
        d_ce.hAlignReal = tmpXY.align;
    } else {
        y = tmpXY.xy;
        resize = d_cc[3] + d_ce.shadowLen != tmpXY.d_sz;
        h = tmpXY.d_sz;
        d_ce.vAlignReal = tmpXY.align;
...
function _dmOl() {
    for (var i = 0; i < d_dm.length; i++) {
        if (d_dm[i] && d_dm[i].m[0].d_sh && dm_menuClicked != i) {
            ...
            while (d_ce && d_ce.d_ct.smHidePause != -1) {
                d_ce = _dmvi(d_ce.d_sh);
            }
            if (d_ce) {
